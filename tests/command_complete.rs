...
#[test]
fn vendor_command_complete_preserves_an_unknown_opcode_and_opaque_payload() {
    let buffer = [0x0E, 6, 1, 0xFF, 0xFF, 0x00, 0x34, 0x12];
    match Event::new(Packet(&buffer)) {
        Ok(Event::CommandComplete(event)) => {
            assert_eq!(event.num_hci_command_packets, 1);
            match event.return_params {
                ReturnParameters::Vendor(params) => {
                    assert_eq!(params.opcode(), hci::Opcode(0xFFFF));
                    assert_eq!(params.status(), hci::Status::Success);
                    assert_eq!(params.payload(), [0x34, 0x12]);
                }
