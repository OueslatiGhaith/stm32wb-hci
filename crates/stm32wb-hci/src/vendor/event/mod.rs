...
macro_rules! decode_hci_event_schema_field {
    ($ty:ty, $len:literal, $data:ident, $original_len:expr) => {
        decode_hci_event_field::<$ty, $len>($data, $original_len)
    };
    (
        $ty:ty,
        {
        ...
        decode_hci_event_counted_bytes::<$ty, $count_ty, $count_len, $max_len>(
            $data,
            $original_len,
        )
    };
    ...
    (
        ...
        decode_hci_event_trailing_bytes::<$ty, $min_len, $max_len>($data)
    };
    (
        $ty:ty,
        {
...
macro_rules! validate_hci_event_schema_field {
    ($ty:ty, $len:literal) => {
        const _: () = {
            let _ = decode_hci_event_field::<$ty, $len>;
        };
    };
    (
        $ty:ty,
        ...
        const _: () = {
            let _ = decode_hci_event_counted_bytes::<$ty, $count_ty, $count_len, $max_len>;
        };
    };
    (
        $ty:ty,
        ...
        const _: () = {
            let _ = decode_hci_event_counted_items::<
                $ty,
                $item_ty,
                $count_ty,
                $count_len,
    ...
    (
        ...
        const _: () = {
            let _ = decode_hci_event_trailing_bytes::<$ty, $min_len, $max_len>;
        };
    };
    (
        $ty:ty,
...
macro_rules! validate_vendor_event_payload {
    ({ Payload = (); }) => {};
    ({
        Payload = {
        ...
        $(validate_hci_event_schema_field!($ty, $shape);)*
    };
}

macro_rules! vendor_event_enum {
    (@collect [$($variants:tt)*]) => {
    ...
    (
        @collect [$($variants:tt)*]
        $(#[$meta:meta])*
        $variant:ident($code:literal) { Payload = (); }
        $($rest:tt)*
    ) => {
        vendor_event_enum! {
            @collect [
    ...
    (
        @collect [$($variants:tt)*]
        $(#[$meta:meta])*
        $variant:ident($code:literal) { Payload = { $($payload:tt)* }; }
        $($rest:tt)*
    ) => {
        vendor_event_enum! {
            @collect [
    ...
    ($($events:tt)*) => {
        vendor_event_enum! { @collect [] $($events)* }
    };
}

macro_rules! vendor_event_payload_type {
...
macro_rules! vendor_event_decode_declarative_fields {
    (
        $variant:ident,
        $payload:ident,
        { $($field:ident: $ty:ty => $shape:tt,)* }
        ...
        Ok(VendorEvent::$variant($variant { $($field,)* }))
    }};
}
...
macro_rules! vendor_event_decode {
    ...
    ($variant:ident, $payload:ident, {
        Payload = { $($field:ident: $ty:ty => $shape:tt,)* };
    }) => {
        vendor_event_decode_declarative_fields!(
            $variant,
            $payload,
            { $($field: $ty => $shape,)* }
        )
...
/// Declares vendor-event IDs, their complete wire payloads, and the typed
/// [`VendorEvent`] values constructed from those payloads.
///
/// Fixed fields use `field: Type => N` and require [`HciEventField<N>`].
/// Variable fields use the same counted, trailing, and semantic-payload shapes
/// as declarative commands. Each non-unit declaration generates its public
/// payload structure directly from `Payload`.
macro_rules! vendor_event {
    ($(
        $(#[doc = $doc:expr])*
        $(#[cfg($cfg:meta)])?
        $variant:ident($code:literal) $body:tt
    )*) => {
        vendor_event_enum! {
            $(
                $(#[doc = $doc])*
        ...
        $(vendor_event_payload_type!($variant, $body);)*
        $($(
            #[cfg($cfg)]
