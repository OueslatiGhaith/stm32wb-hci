...
//! Rust method that intentionally maps to an already claimed ST command.
...
/// Finds the next Rust method after a marker comment.
///
...
fn next_method_name(lines: &[&str], start: usize) -> Option<String> {
    for line in lines.iter().skip(start) {
        ...
        if let Some(method) = super::rust_method::parse_fn_name(trimmed) {
            return Some(method);
        }
    }
